...
export class OrganizationDetailsController {
  ...
  constructor(codenvyResourcesDistribution: CodenvyResourcesDistribution, codenvyPermissions: CodenvyPermissions,
              ...
              lodash: any, codenvyOrganization: CodenvyOrganization, $timeout: ng.ITimeoutService) {
  ...
  updateSelectedTab(): void {
    ...
    this.selectedTabIndex = tab ? parseInt(Tab[tab], 10) : 0;
  }
  ...
  isUserAllowedTo(value: string): boolean {
    if (value === 'update' && this.isPersonalOrganization()) {
      return false;
    }
  ...
  canChangeResourceLimits(): boolean {
    ...
    return this.organization && this.isUserAllowedTo('manageResources');
  }
